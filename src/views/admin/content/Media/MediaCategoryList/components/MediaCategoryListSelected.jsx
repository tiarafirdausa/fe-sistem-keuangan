...
const MediaCategoryListSelected = () => {
    ...
    const handleConfirmDelete = async () => {
        ...
        try {
            await Promise.all(
                selectedMediaCategories.map((category) => apiDeleteMediaCategory(category.id)) 
            );
            mutate();
            setSelectAllMediaCategories([]); 
            ...
            toast.push(
                <div className="flex items-center">
                    ...
                    />
                    <span>Successfully deleted {selectedMediaCategories.length} media category(s)!</span>
                </div>
            );
        } catch (error) {
            console.error("Error deleting media categories:", error);
            toast.push(
                <div className="flex items-center">
                    <Avatar
                        shape="circle"
                    ...
                    />
                    <span>Failed to delete media category(s). Please try again.</span>
                </div>
            );
        }
    };
    ...
    return (
        <>
                                    ...
                                    <span className="flex items-center gap-2">
                                        ...
                                        <span className="font-semibold flex items-center gap-1">
                                            <span className="heading-text">
                                                {selectedMediaCategories.length}{' '}
                                                Category(s)
                                            </span>
                                            <span>selected</span>
                                        </span>
            ...
            >
                <p>
                    Are you sure you want to remove these media categories? This action
                    can&apos;t be undone.
                </p>
            </ConfirmDialog>
        </>
...
export default MediaCategoryListSelected;